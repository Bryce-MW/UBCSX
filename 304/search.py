...
from ubcsx import names, symbols, script_html, params, current_page, urlencode, cursor, escape
...
if query:
    ...
    for row in cursor:
        rows += 1
        search_results += search_result.format(symbol_url=urlencode({"symbol": row['symbol']}), symbol=escape(row["symbol"]), name=escape(row["name"]), last=row["last"], bid=row["bid"] or 0, ask=row["ask"] or 0)
